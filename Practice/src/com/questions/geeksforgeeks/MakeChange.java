        ...
        for( int cents = 1; cents <= maxChange; cents++ )
        {
            ...
            for( int j = 0; j < differentCoins; j++ )
            {
                if( coins[ j ] > cents )   // Cannot use coin j
                    continue;
                System.out.println( "coinsUsed["+(cents - coins[ j ])+"] = "+(coinsUsed[ cents - coins[ j ] ]));
            ...
            System.out.println(cents+" : "+Arrays.toString(coinsUsed));
            System.out.println(cents+" : "+Arrays.toString(lastCoin));
        }
    }
